            ...
            switch (evt.info.code) {
                case 'NetConnection.Connect.Success':
                    if (evt.info.secureToken != undefined) {
                        _connection.call("secureTokenResponse", null,
                            TEA.decrypt(evt.info.secureToken,'lwPnJvHfp0tqp7OX'));
                    }
                    if (evt.info.data) {
                ...
                case 'NetStream.Play.StreamNotFound':
                    if (!_isStreaming) {
                        ...
                        sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_ERROR, {message: "Waiting for the feed..."});
                    }
                    break;
				...
				case 'NetConnection.Connect.Failed':
					if(item.streamer.substr(0,5) == 'rtmpt') {
						stop();
						sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_ERROR, {message: "Video server down"});
					} else {
						_responded = false;
					}
					break;
